...
// public methods
//----------------------------------------------------------------------------------------
const ParentMatcher = /\$parent\./g

Context.prototype.generateVariable = function(name) {
    var arr = [], parentLevels = 0;

    if (typeof name !== 'undefined') {
        parentLevels = (name.match(ParentMatcher) || []).length; // Count number of parent parts
        if (parentLevels > 0) name = name.replace(ParentMatcher, '') // Remove parent parts
    }

    Array.prototype.push.apply(arr, this.scopes[this.scopes.length - 1]);
    arr = arr.slice(0, arr.length - parentLevels);
    if (name) {
        arr.push(name);
    }

    console.log('GenVar: '+ arr.join('.'));
    return arr.join('.');
};
